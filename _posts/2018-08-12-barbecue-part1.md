...
Incidentally, although my kettle has air vents in the lid and the base I never use these to control the heat (and thus the cooking speed). I just leave the vents fully open and control the heat - as I've already described - by positioning the coals and food appropriately. My reasoning here is that continually tweaking the vents forces a micromanagement approach which is incompatible with my hands-off cooking style and the kettle's very slow reponse times to control inputs. In any case, my kettle doesn't have a thermometer, so I don't have the data to decide when to open or close the vents anyway. In contrast, my smoker does have a thermometer and I do sometimes change its air vents. But I mainly check the smoker's thermometer only to decide when to add more coals (something I never do with my kettle) and I only close the vents if I have got the heat stupidly high. I don't try to make continual control inputs with the vents, again because I want to avoid micromanagement, and because it's a smoker, not an induction hob.
